...
def time_solver(func=None, *,multi_run=True):
    ...
    def decorator(f):
        def wrapper(*args, **kwargs):
            ...
            if not multi_run or first_run >= 1.0:
            ...
            if iterations > 1:
                if stdev / avg_time > 0.1:  # High variance (>10%)
                    print(f"[{f.__name__}] Time: {time_str} (avg of {iterations} runs, ±{stdev/avg_time*100:.1f}%)")
                else:
                    print(f"[{f.__name__}] Time: {time_str} (avg of {iterations} runs)")
            else:
                print(f"[{f.__name__}] Time: {time_str}")
            
            return result
        return wrapper
    if func is not None:
        return decorator(func)
    # If called with parentheses: @time_solver(multi_run=False)
    else:
        return decorator